...
const CaseStudyArticle = () => {
  ...
  useEffect(() => {
    const loadArticle = async () => {
      setLoading(true);
      const found = await getArticleBySlug("case-studies", slug);
      setArticle(found);
      setLoading(false);
    };
  ...
  useEffect(() => {
    if (article && articleRef.current) {
      setTimeout(() => {
        if (articleRef.current) initializeArticleEnhancements(articleRef.current);
      }, 100);
    }
  }, [article]);
            ...
            <aside className="lg:col-span-4 space-y-6 sticky top-8">
              ...
              {article.outcome && (
                <Card className="border-primary/20 bg-primary/5">
                  <CardHeader><CardTitle className="text-primary">Key Outcome</CardTitle></CardHeader>
                  <CardContent><p className="text-sm text-foreground">{article.outcome}</p></CardContent>
                </Card>
              )}

              {article.challenge && (
                <Card>
                  <CardHeader><CardTitle>Main Challenge</CardTitle></CardHeader>
                  <CardContent><p className="text-sm text-muted-foreground">{article.challenge}</p></CardContent>
                </Card>
              )}
            ...
            <CardContent className="p-10 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button asChild size="lg" className="min-w-[200px]">
                  <Link to="/contact">Reach Out to Us Today<ArrowLeft className="w-4 h-4 ml-2 rotate-180" /></Link>
                </Button>
