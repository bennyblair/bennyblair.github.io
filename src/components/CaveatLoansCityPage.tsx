...
const securityPoints = [
  'Residential, commercial, or mixed-use property with sufficient equity may support a caveat loan depending on title position and lender appetite.',
  'Caveat lenders typically focus on security quality, speed, and exit realism more than full long-form bank-style servicing analysis.',
  'A short-term caveat structure usually needs a clearly defined exit such as refinance, sale, debtor receipt, settlement completion, or another commercial event.',
  'Because caveat lending is urgency-driven, clean title, document readiness, and fast legal coordination materially affect execution speed.'
];

const processSteps = [
  'Confirm the urgency, property security, current debt position, and exactly what the funding must solve.',
  'Assess lender fit based on property type, title position, leverage, and whether the exit is realistic within the proposed term.',
  'Coordinate valuation, solicitor engagement, and core documents quickly so the file can move without avoidable legal or administrative friction.',
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <SEO
        ...
        canonical={canonical}
        keywords={`caveat loans ${city}, urgent caveat finance ${city}, property secured business loan ${city}, short term property finance ${city}`}
        schemas={[generateServiceSchema(`Caveat Loans ${city}`, description, `https://emetcapital.com.au${canonical}`)]}
      />
      ...
      <div className="container mx-auto px-4 py-8">
        <Breadcrumbs
          items={[
            ...
            { label: 'Caveat Loans', href: '/services/caveat-loans' },
            { label: city }
          ]}
        />

        <section className="max-w-5xl mx-auto text-center pt-4 pb-12">
          <Badge className="mb-4 bg-sky-500/15 text-sky-300 border border-sky-500/30">{city} Caveat Finance</Badge>
          <div className="flex items-center justify-center gap-3 mb-4">
            <MapPin className="h-8 w-8 text-sky-300" />
            <h1 className="text-4xl lg:text-5xl font-bold text-white">Caveat Loans {city}</h1>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" asChild className="bg-sky-500 hover:bg-sky-400 text-slate-950">
              <Link to="/contact">Discuss your urgent scenario</Link>
            </Button>
        ...
        <section className="max-w-5xl mx-auto grid lg:grid-cols-2 gap-6 mb-12">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-8">
            <h2 className="text-2xl font-bold text-white mb-4">How caveat loans may fit {city} situations</h2>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-8">
            ...
            <div className="space-y-4 text-slate-300">
              <p><span className="font-semibold text-white">Urgency:</span> caveat lending is usually considered when the funding need is immediate and a slower bank process is unlikely to help in time.</p>
              <p><span className="font-semibold text-white">Security:</span> property equity and title position matter because the lender is relying on short-term property-backed security rather than a full-term mortgage structure.</p>
              <p><span className="font-semibold text-white">Exit:</span> the strongest files still show a believable repayment pathway instead of relying on “we will figure it out later.”</p>
              <p><span className="font-semibold text-white">Execution:</span> if valuations, legal work, or title issues are messy, settlement can still slow down despite the product being designed for speed.</p>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-8">
            <h2 className="text-3xl font-bold text-white mb-8 text-center">What caveat lenders usually care about in {city}</h2>
            <div className="grid md:grid-cols-2 gap-6">
              {securityPoints.map((item) => (
                <div key={item} className="flex gap-4 items-start bg-slate-950 rounded-lg border border-slate-800 p-5">
                  <Shield className="h-5 w-5 text-sky-300 mt-0.5 shrink-0" />
        ...
        <section className="max-w-5xl mx-auto mb-12">
          <h2 className="text-3xl font-bold text-white mb-8 text-center">Common {city} caveat-loan use cases</h2>
          <div className="grid md:grid-cols-2 gap-6">
        ...
        <section className="max-w-4xl mx-auto text-center pb-12">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 mb-6">
            <h2 className="text-3xl font-bold text-white mb-4">Need an urgent {city} caveat-loan solution?</h2>
            <p className="text-slate-300 mb-6 max-w-2xl mx-auto leading-relaxed">
              If timing is critical and the file has strong property-backed security, a caveat loan may help bridge the gap while a refinance, sale, settlement, or business event catches up. We can help assess whether the scenario is genuinely workable and which lenders may fit it.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" asChild className="bg-sky-500 hover:bg-sky-400 text-slate-950">
              ...
              <Button size="lg" variant="outline" asChild className="border-slate-700 text-slate-100 hover:bg-slate-900">
                <Link to="/services/caveat-loans">Explore caveat loans</Link>
              </Button>
            </div>
          </div>
