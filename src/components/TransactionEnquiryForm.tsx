...
export default function TransactionEnquiryForm({ formName, className = "" }: { formName: "contact" | "homepage-contact"; className?: string }) {
  ...
  return (
    <form name={formName} method="POST" data-netlify="true" data-netlify-honeypot="bot-field" onSubmit={handleSubmit} className={"space-y-6 " + className}>
      ...
      <p className="text-sm text-muted-foreground" aria-live="polite">{"Step " + step + " of 2 · " + (step === 1 ? "Your transaction" : "Your contact details")}</p>
      ...
      <fieldset hidden={step !== 2} disabled={step !== 2} className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Button type="button" variant="outline" disabled={submitting} onClick={() => { setStep(1); window.setTimeout(() => purposeRef.current?.focus(), 0); }}>Back</Button>
