...
export default function TransactionEnquiryForm({ formName, className = "" }: { formName: "contact" | "homepage-contact"; className?: string }) {
  ...
  const [purpose, setPurpose] = useState(() => typeof document === "undefined" ? "" : (document.querySelector(`main form[name="${formName}"] select[name="transactionPurpose"]`) as HTMLSelectElement | null)?.value || "");
  const [fields, setFields] = useState(() => Object.fromEntries(Object.entries(emptyFields).map(([key, value]) => [key, typeof document === "undefined" ? value : (document.querySelector(`main form[name="${formName}"] [name="${key}"]`) as HTMLInputElement | null)?.value || value])) as typeof emptyFields);
  ...
  useEffect(() => {
    ...
    if (initial !== "not_provided" && !purposeRef.current?.value) setPurpose(initial);
  ...
  const next = () => {
    ...
    window.setTimeout(() => contactRef.current?.focus(), 0);
  };
  ...
  return (
    <form name={formName} method="POST" action="/" data-enquiry-step={step} data-netlify="true" data-netlify-honeypot="bot-field" onSubmit={handleSubmit} onKeyDown={(event) => { if (step === 1 && event.key === "Enter" && (event.target as HTMLElement).closest('[data-enquiry-panel="1"]')) { event.preventDefault(); next(); } }} className={"transaction-enquiry space-y-6 " + className}>
      ...
      <p className="enquiry-enhanced-control text-sm text-muted-foreground" aria-live="polite">{"Step " + step + " of 2 · " + (step === 1 ? "Your transaction" : "Your contact details")}</p>

      <fieldset data-enquiry-panel="1" className="space-y-5">
        ...
        <Button type="button" onClick={next} className="enquiry-enhanced-control w-full min-h-11">Continue to contact details</Button>
      </fieldset>

      <fieldset data-enquiry-panel="2" className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Button type="button" className="enquiry-enhanced-control" variant="outline" disabled={submitting} onClick={() => { setStep(1); window.setTimeout(() => purposeRef.current?.focus(), 0); }}>Back</Button>
