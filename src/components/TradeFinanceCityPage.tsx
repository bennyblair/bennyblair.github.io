    ...
    <div className="location-page min-h-screen bg-background text-foreground">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="page-header location-header max-w-5xl mx-auto text-center pt-4 pb-12">
          <Badge className="mb-4 bg-primary/10 text-primary border border-border">{city} Trade Finance</Badge>
          <div className="location-title-row flex items-center justify-center gap-3 mb-4"><MapPin className="h-8 w-8 text-primary" /><h1 className="text-4xl lg:text-5xl font-bold text-foreground">Trade Finance {city}</h1></div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center"><Button size="lg" asChild className="bg-primary hover:bg-primary/90 text-primary-foreground"><Link to="/contact">Discuss your trade cycle</Link></Button><Button size="lg" variant="outline" asChild className="border-border text-foreground hover:bg-card"><Link to="/services/trade-finance"><ArrowLeft className="mr-2 h-5 w-5" />Back to Trade Finance</Link></Button></div>
        </section>
        <section className="max-w-5xl mx-auto grid lg:grid-cols-2 gap-6 mb-12"><div className="bg-card border border-border rounded-xl p-8"><h2 className="text-2xl font-bold text-foreground mb-4">How trade finance may fit {city} businesses</h2><p className="text-muted-foreground leading-relaxed mb-4">{description}</p><p className="text-muted-foreground leading-relaxed">{localFocus}</p></div><div className="bg-card border border-border rounded-xl p-8"><h2 className="text-2xl font-bold text-foreground mb-4">Why timing pressure shows up in {city}</h2><p className="text-muted-foreground leading-relaxed mb-4">{timingPressures}</p><div className="space-y-4 text-muted-foreground"><p><span className="font-semibold text-foreground">Trade timing:</span> supplier payments, shipment deadlines, customs timing, and customer payment cycles can all create funding gaps.</p><p><span className="font-semibold text-foreground">Facility fit:</span> the right structure depends on whether the pressure sits in imports, exports, receivables, inventory, or counterpart risk.</p><p><span className="font-semibold text-foreground">Commercial purpose:</span> trade lenders still want to understand the trade cycle and the economic purpose of the transaction.</p><p><span className="font-semibold text-foreground">Execution:</span> missing shipment windows or supplier deadlines can cost more than the finance itself if timing is handled poorly.</p></div></div></section>
        <section className="max-w-5xl mx-auto mb-12"><h2 className="text-3xl font-bold text-foreground mb-8 text-center">Suburbs, precincts, and trade corridors we watch in {city}</h2><div className="grid md:grid-cols-3 gap-6">{suburbCoverage.map((item) => <div key={item.title} className="bg-card border border-border rounded-xl p-6"><div className="flex items-center gap-3 mb-3"><Building2 className="h-5 w-5 text-primary" /><h3 className="text-xl font-semibold text-foreground">{item.title}</h3></div><p className="text-muted-foreground leading-relaxed">{item.text}</p></div>)}</div></section>
        <section className="max-w-5xl mx-auto mb-12"><div className="bg-card border border-border rounded-xl p-8"><h2 className="text-3xl font-bold text-foreground mb-8 text-center">What trade-finance lenders usually care about in {city}</h2><div className="grid md:grid-cols-2 gap-6">{lenderPoints.map((item) => <div key={item} className="flex gap-4 items-start bg-background rounded-lg border border-border p-5"><Globe className="h-5 w-5 text-primary mt-0.5 shrink-0" /><p className="text-muted-foreground leading-relaxed">{item}</p></div>)}</div></div></section>
        <section className="max-w-5xl mx-auto mb-12"><h2 className="text-3xl font-bold text-foreground mb-8 text-center">Common {city} trade-finance use cases</h2><div className="grid md:grid-cols-2 gap-6">{localUseCases.map((item) => <div key={item.title} className="bg-card border border-border rounded-xl p-6"><h3 className="text-xl font-semibold text-foreground mb-3">{item.title}</h3><p className="text-muted-foreground leading-relaxed">{item.text}</p></div>)}</div></section>
