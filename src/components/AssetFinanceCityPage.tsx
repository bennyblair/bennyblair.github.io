    ...
    <div className="location-page min-h-screen bg-background text-foreground">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="page-header location-header max-w-5xl mx-auto text-center pt-4 pb-12">
          <Badge className="mb-4 bg-primary/10 text-primary border border-border">{city} Asset Finance</Badge>
          <div className="flex items-center justify-center gap-3 mb-4">
            <MapPin className="h-8 w-8 text-primary" />
            <h1 className="text-4xl lg:text-5xl font-bold text-foreground">Asset Finance {city}</h1>
          </div>
        ...
        <section className="max-w-5xl mx-auto grid lg:grid-cols-2 gap-6 mb-12">
          <div className="bg-card border border-border rounded-xl p-8">
            <h2 className="text-2xl font-bold text-foreground mb-4">How asset finance may fit {city} businesses</h2>
          ...
          <div className="bg-card border border-border rounded-xl p-8">
            ...
            <div className="space-y-4 text-muted-foreground">
              <p><span className="font-semibold text-foreground">Asset type:</span> vehicles, machinery, medical equipment, hospitality plant, and technology all attract different lender appetites and terms.</p>
              <p><span className="font-semibold text-foreground">Structure choice:</span> chattel mortgage, lease, hire purchase, and related structures each change cash flow, accounting, and ownership outcomes.</p>
              <p><span className="font-semibold text-foreground">Supplier timing:</span> settlement often needs to line up with delivery windows, stock availability, or business rollout timing.</p>
              <p><span className="font-semibold text-foreground">Commercial purpose:</span> the strongest files explain exactly how the asset will support growth, efficiency, or replacement rather than treating finance as a generic commodity.</p>
            </div>
          </div>
        </section>

        <section className="max-w-5xl mx-auto mb-12">
          <h2 className="text-3xl font-bold text-foreground mb-8 text-center">Suburbs, precincts, and business corridors we watch in {city}</h2>
        ...
        <section className="max-w-5xl mx-auto mb-12">
          <div className="bg-card border border-border rounded-xl p-8">
            <h2 className="text-3xl font-bold text-foreground mb-8 text-center">Common asset types financed in {city}</h2>
            <div className="grid md:grid-cols-2 gap-6">
              {assetTypes.map((item, idx) => (
                <div key={item} className="flex gap-4 items-start bg-background rounded-lg border border-border p-5">
                  {idx % 2 === 0 ? <Wrench className="h-5 w-5 text-primary mt-0.5 shrink-0" /> : <Truck className="h-5 w-5 text-primary mt-0.5 shrink-0" />}
        ...
        <section className="max-w-5xl mx-auto mb-12">
          <h2 className="text-3xl font-bold text-foreground mb-8 text-center">Common {city} asset finance use cases</h2>
          <div className="grid md:grid-cols-2 gap-6">
        ...
        <section className="max-w-4xl mx-auto text-center pb-12">
          <div className="bg-card border border-border rounded-xl p-8 mb-6">
            <h2 className="text-3xl font-bold text-foreground mb-4">Need an asset finance solution in {city}?</h2>
